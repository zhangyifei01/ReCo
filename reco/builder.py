...
class MoCo(nn.Module):
    ...
    def __init__(self, base_encoder, dim=128, K=65536, m=0.999, T=0.07, TT=0.04, ST=0.1, mlp=False):
        ...
        self.TT = TT
        self.ST = ST
        self.alpha = 1.0  # beta distribution
        ...
        if mlp:  # hack: brute-force replacement
            ...
            # self.encoder_q.fc = nn.Sequential(nn.Linear(dim_mlp, dim_mlp), nn.BatchNorm1d(dim_mlp), nn.ReLU(), nn.Linear(dim_mlp, dim_mlp), nn.ReLU(), self.encoder_q.fc)
            # self.encoder_k.fc = nn.Sequential(nn.Linear(dim_mlp, dim_mlp), nn.BatchNorm1d(dim_mlp), nn.ReLU(), nn.Linear(dim_mlp, dim_mlp), nn.ReLU(), self.encoder_k.fc)
    ...
    #     self.alpha = round(max(math.exp(-math.log(0.07 * epoch + 1) + 1.6), 0.5), 2)
    #     print(self.alpha)
    ...
    def _img_interpolation(self, im_q, im_k):
        ...
        choose = 1
        ratio = np.random.beta(self.alpha, self.alpha)  #####1

        if choose == 0:  # mixup
            im_mix = ratio * im_q + (1 - ratio) * im_k[noise]
        else:  # cutmix
            bbx11, bby11, bbx12, bby12 = self._rand_bbox(im_q.size(), ratio)
            true_ratio = ((bbx12 - bbx11) * (bby12 - bby11) / (im_q.size(2) * im_q.size(3)))
            # while true_ratio < 1./196:
    ...
    def forward(self, im_q, im_k, im_k_w):
        ...
        # feature of interpolated images
        ...
        # feature of interpolated feature
        reordered_k = k[noise]
