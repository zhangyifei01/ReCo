...
writer = SummaryWriter('runs/reco')
...
parser.add_argument('--warmup', default=5, type=int,
                    help='warm-up epochs (default: 10)')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.aug_plus:
        ...
        augmentation = [
            ...
            transforms.RandomApply([reco.loader.GaussianBlur([.1, 2.])], p=0.5),
    ...
    train_dataset = datasets.ImageFolder(
        traindir,
        reco.loader.TwoCropsTransform(transforms.Compose(augmentation), transforms.Compose(augmentation_target)))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        # model.module.set_alpha(epoch)  # set alpha
        if args.distributed:
...
def adjust_learning_rate(optimizer, epoch, args, i, iteration_per_epoch):
    ...
    if epoch < args.warmup:
        lr = args.lr * 1.0 * T / warmup_iters
    else:
        T = T - warmup_iters
        lr = 0.5 * args.lr * (1. + math.cos(1.0 * T / total_iters * math.pi))
